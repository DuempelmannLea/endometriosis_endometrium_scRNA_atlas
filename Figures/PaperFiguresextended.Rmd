...
Figure 1c
UMAP of the entire endometrium single-cell atlas from women with and without endometriosis, colored by cell types (n=63).
```{r}
...
Figure 1d
Main cell type frequencies per patient (n=60, box plot, left) and entire dataset (bar plot, middle), with mesenchymal cells being the most abundant cell type. The overall cell number distribution between non-ENDO, mild-ENDO and severe-ENDO is balanced (bar plot, right).
```{r plotting of cell types per sample}
## Fig1d_1 Cells per patient
# Boxplot
...
## Fig1d_2 Cells per main cell type
# Barplot
...
##Fig1d_3 cell number distribution between non-ENDO, mild-ENDO and severe-ENDO
# Barplot
...
Figure 1e
Eigencor plot showing the effect of different co-variants of the atlas dataset on principal components (PCs) with Pearson correlation values and multiple-testing adjustment with the Benjamini-Hochberg procedure.
...
Figure 2a
Scores plot of the first two principal components from PCA of sample-wise whole transcriptome.
```{r}
## Figure 2a
# Load data
p <- readRDS(paste0(dir_ENDO,"_Data/03_PCA_analysis/p_EndoAtlas.rds"))
...
Figure 2b
UMAPs of the entire dataset (left) and the re-integrated epithelial cells (right), colored by menstrual cycle phase, show a clear visual separation between proliferative/periovulatory, early-, mid-, and late-secretory samples, particularly in mesenchymal (endometrial fibroblasts, C7 fibroblasts, and mural cells), epithelial (glandular), and endothelial cells.
```{r}
...
Figure 2c
UMAP of the re-integrated epithelial cells, colored by pseudo time prediction from monocle3.
```{r}
# Figure 2c epithelial cells
epithelial_cells_monocle3 <- readRDS(paste0(dir_data, "epithelial_cells_monocle3.rds"))

# DimPlot of epithelial cells with pseudotime
...
Figure 2d
Heatmap of selected marker gene expression with high log fold changes over the key menstrual cycle phases (proliferative, periovulatory, early-, mid-, and late-secretory).
```{r}
...
#### Plot heatmaps for main cell type specific markers for the different menstrual cycle phases. Compare also to Wang et al 2020 marker genes (Wang et al. 2020, DOI: 10.1038/s41591-020-1040-z)
...
Figure 2e
Boxplots of sample-wise mesenchymal and epithelial cell frequency as a percentage of total cells, split by menstrual cycle phase (stringent patient exclusion criteria, 32 ENDO, and 22 non-ENDO samples).
```{r plotting of cell types per sample}
...
Fig2e <- ggplot(df_Fig2e, aes(x = MenstrualCyclePhase_main, y = AnnotationMain_perc, fill = EndometriosisStatus)) +
...
df_Fig2e_ttest_EndometriosisStatus <- df_Fig2e %>%
...
df_Fig2e_ttest_ProlifPeri <- df_Fig2e %>%
...
df_Fig2e_ttest_ProlifSec <- df_Fig2e %>%
...
df_Fig2e_ttest_PeriSec <- df_Fig2e %>%
...
Make better comments! xxx
Figure 2f
Boxplot of sample-wise secretory mural cell frequency as a percentage of total cells.
```{r plotting of cell types per sample}
#Early and Mid Secretory Prv_VSMCs 
df_Fig2f <- EndoAtlas_meta%>%
  filter(Minor.exclusion.criteria.logical == "FALSE") %>%
...
df_Fig2f <- left_join(df_Fig2f, dplyr::distinct(EndoAtlas_meta[,c("sample","EndometriosisStatus","MenstrualCyclePhase_main","MenstrualCyclePhase")]), by = "sample")

secEarylMid_Prv_VSMC_MenstrualCyclePhase_main <- df_Fig2f %>%
...
Figure 3a and 3b
3a Barplot showing number of unique differentially expressed genes (DEGs, adjusted p-value < 0.05, average expression > 18.53202) between non-ENDO (n = 11) and all-ENDO (n = 12), mild-ENDO (n = 4) and severe-ENDO (n = 8) samples within immune, endothelial, epithelial, and mesenchymal cell types, analyzed by Muscat.
3b Selected terms enriched in the cluster- and stage-wise functional enrichment analysis of DEGs, with term sizes between 25 and 750.
```{r}
...
xxx
Figure 3c-h
(c) Selected signaling pathways with overall enriched activity in ENDO (n = 12) in comparison to non-ENDO (n = 11).
(d) Circle plot of TNF signaling pathway between all cell types in ENDO (left, n = 12) and non-ENDO (right, n = 11).
(e-h) Bubble plots of selected communications with significant interaction probabilities (unadjusted p-value < 0.01) within the ICAM (e), CXCL (f), IGFBP (g) and VEGF (h) signaling pathways for ENDO (n = 12) and non-ENDO (n = 11). Further p-value adjustment is not recommended by the CellChat author. 
```{r}
...
Violin plots illustrating counts and features from RNA assay (a) and SCT assay (b), show no discernable differences between ENDO and non-ENDO.
...
Supplementary Data Figure 1c and d
UMAPs of the entire endometrium single-cell atlas, colored by menstrual cycle day (c) and progesterone in serum (d), show a clear visual separation of cells by menstrual cycle day and serum progesterone concentration. 
```{r}
## SFig1c
# Featureplot
...
xxx double-check code
Supplementary Data Figure 1e
Eigencor plot showing the effect of different co-variants of the atlas dataset on principal components (PCs) of the proliferative samples with strict exclusion criteria with Pearson correlation values and multiple-testing adjustment with the Benjamini-Hochberg procedure.
```{r}
# Set dir_out
...
Eigencor plots showing the effect of different co-variants of the atlas dataset on principal components (PCs) of the main 5 cell types (epithelial, mesenchymal, endothelial, myeloid, lymphoid) with Pearson correlation values and multiple-testing adjustment with the Benjamini-Hochberg procedure. 
...
Expression of the marker genes for the different cell types corresponds to the cell type-specific expression shown in Tan et al. 2022 (DOI: 10.1038/s41556-022-00961-5).
...
Scores plot of the first two principal components PC1 and PC2 from sample-wise whole transcriptome PCA of the SCT assay, colored by endometriosis status.
```{r}
# Load data
p <- readRDS(paste0(dir_ENDO,"_Data/03_PCA_analysis/p_EndoAtlas.rds"))
# biplot
...
# save biplot
ggsave(plot = SFig3a, "./SFig3a.pdf", width = 7, height = 7)
```

Supplementary Figure 3b and e
UMAP of re-integrated epithelial cells colored by menstrual cycle day and menstrual cycle phase Wang. UMAP of the entire dataset colored by the Symphony transferred Wang phases, show a clear visual separation between phase 1/2, phase 3, and phase 4. 
```{r}
# Load data
epithelial_cells_monocle3 <- readRDS(paste0(dir_data, "epithelial_cells_monocle3.rds"))

#Plot UMAP with Menstrual Cycle Day
...
#Plot UMAP with Menstual Cycle Phase Wang
...
Scatter plots showing the correlation of menstrual cycle day and serum progesterone with our menstrual cycle staging (c) and median Wang phases (d), transferred with Symphony (d).
```{r}
xxx missing
...
UMAP of the entire dataset colored by the Symphony transferred Wang phases, show a clear visual separation between phase 1/2, phase 3, and phase 4. 
```{r}
## Figure 2b UMAP entire atlas
Fig2b_1 <- DimPlot(EndoAtlas, reduction = "umap", group.by = "WangMenstrualCyclePhase", raster = FALSE)
Fig2b_1
# Save UMAP
ggsave(filename = paste0(dir_out, 'Fig2b_1.pdf'),
       plot = Fig2b_1,
...
Heatmap showing the expression of menstrual cycle phase markers (as in Fig. 2d) of the eutopic endometrial epithelial cells (g) from the Tan et al. 2022 single-cell atlas.
```{r}
...
Heatmaps showing the expression of menstrual cycle phase markers (as in Fig. 2d) of the eutopic endometrial mesenchymal cells (h) from the Tan et al. 2022 single-cell atlas.
```{r}
####### Download data from Tan et al. 2022 (DOI: 10.1038/s41556-022-00961-5) through the command line
#cd endometriosis_endometrium_scRNA_atlas/_Data
#wget --no-check-certificate https://singlecell.jax.org/datasets/endometriosis/h5ad/endo-2022_epithelial.h5ad
...
Bar plot of cell type frequencies per sample, sorted by menstrual cycle phase and pseudo time and divided by the main 5 cell types. 
```{r plotting of cell types per sample}
...
#Make bar plot
SFig4a <- ggplot(SFig4a_df, aes(x = sample, y = row_count, fill = AnnotationRefined)) +
  geom_bar(stat = 'identity', position = 'fill') +
  scale_y_continuous(expand = c(0, 0)) +
...
Box plots of refined cell type frequencies between the main menstrual cycle phases. 
```{r plotting of cell types per sample}
##########################################################################################
...
Genes underlying selected pathways enriched in the cluster- and stage-wise functional enrichment analysis.
```{r}
DEGs
...
(b) Signaling pathways with relative information flow from CellChat analysis, ranked by overall information flow within the inferred networks between ENDO and non-ENDO.
(c) Circle plot of TGFb signaling pathway between all cell types in ENDO (top) and non-ENDO (bottom, no significant interactions).
(d) Bubble plot of selected communication probabilities with significant interaction probabilities (unadjusted p-value < 0.01)  within the TNF signaling pathway.
```{r}
#Load CellChat data
cellchat <- readRDS(paste0(dir_out,"cellchat_CntrlvsEndo_Symphony_Refined_Final.rds"))
