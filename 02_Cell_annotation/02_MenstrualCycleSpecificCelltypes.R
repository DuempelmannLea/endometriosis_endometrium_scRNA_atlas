...
library(PCAtools)
library(scCustomize)
dir_out <- "../_Data/02_Cell_annotation/"
...
DimPlot_scCustom(data, reduction = "umap", group.by = "AnnotationMain")
DimPlot_scCustom(data, reduction = "umap", group.by = "AnnotationRefined")
...
DimPlot(data, label=T,reduction = "umap", group.by = "EndometriosisStatus", cells.highlight = cellshighlight, cols.highlight = c("darkred"), cols= "grey", raster = FALSE)
...
table(cluster2$AnnotationRefined)
...
DimPlot(cluster2, group.by = "AnnotationRefined")
DimPlot(cluster2, group.by = "AnnotationRefined", split.by = "MenstrualCyclePhase")
DimPlot(cluster2, group.by = "MenstrualCyclePhase", split.by = "AnnotationRefined")

#Cluster2 "dS1-myofibroblast","dS2","eF1","eF2","eF3" become dS3 (decidualized stromal 3)
meta_final <- mutate(meta_final,
                     AnnotationRefined_Final = case_when(
                       integrated_snn_res.0.1 == 2 & AnnotationRefined_Final %in% c("dS1-myofibroblast", "dS2", "eF1", "eF2", "eF3") ~ "dS3",
                       TRUE ~ AnnotationRefined_Final
                     ))
table(meta_final$AnnotationRefined_Final) #sanity check

#Cluster2 "fib C7","fib C7-SFRP2" become "fib C7 secretory"
meta_final <- mutate(meta_final,
                     AnnotationRefined_Final = case_when(
                       integrated_snn_res.0.1 == 2 & AnnotationRefined_Final %in% c("fib C7","fib C7-SFRP2") ~ "fib C7 secretory",
                       TRUE ~ AnnotationRefined_Final
                     ))
table(meta_final$AnnotationRefined_Final) #sanity check
...
table(cluster7_14$AnnotationRefined) #corresponds mainly to eF3 (7067), dS2 (1550) and dS1 (1737)
#Fits nicely, samples 104,131 and 87 high numbers

DimPlot(cluster7_14, group.by = "AnnotationRefined")
DimPlot(cluster7_14, group.by = "AnnotationRefined", split.by = "MenstrualCyclePhase")
DimPlot(cluster7_14, group.by = "MenstrualCyclePhase", split.by = "AnnotationRefined")
table(data$AnnotationRefined)

table(meta_final$AnnotationRefined_Final)
#Cluster7_14 "dS1-myofibroblast","dS2","eF1","eF2","eF3" become dS4
meta_final <- mutate(meta_final,
                     AnnotationRefined_Final = case_when(
                       integrated_snn_res.0.1 %in% c(7, 14) & AnnotationRefined_Final %in% c("dS1-myofibroblast", "dS2", "eF1", "eF2", "eF3") ~ "dS4",
                       TRUE ~ AnnotationRefined_Final
                     ))
table(meta_final$AnnotationRefined_Final) #sanity check
...
meta_final <- mutate(meta_final,
                     AnnotationRefined_Final = case_when(
                       integrated_snn_res.0.1 %in% c(7, 14) & AnnotationRefined_Final %in% c("fib C7","fib C7-SFRP2") ~ "fib C7 secretory_late",
                       TRUE ~ AnnotationRefined_Final
                     ))
table(meta_final$AnnotationRefined_Final) #sanity check
...
meta_final <- mutate(meta_final,
                     AnnotationRefined_Final = case_when(
                       integrated_snn_res.0.1 %in% c(7, 14) & AnnotationRefined_Final %in% c("Prv-CCL19","Prv-MYH11", "Prv-STEAP4" , "VSMC") ~ "Prv_VSMC secretory_late",
                       TRUE ~ AnnotationRefined_Final
                     ))
table(meta_final$AnnotationRefined_Final) #sanity check

table(meta_final$AnnotationRefined_Final,meta_final$integrated_snn_res.0.1)
...
table(cluster8$AnnotationRefined)
...
DimPlot(cluster8, group.by = "AnnotationRefined") 
DimPlot(cluster8, group.by = "MenstrualCyclePhase") +
  DimPlot(cluster8, group.by = "integrated_snn_res.1")
DimPlot(cluster8, group.by = "AnnotationRefined", split.by = "MenstrualCyclePhase")
DimPlot(cluster8, group.by = "MenstrualCyclePhase", split.by = "AnnotationRefined")
...
DimPlot_scCustom(cluster8, reduction = "umap", group.by = "AnnotationRefined")
...
table(cluster8_1$AnnotationRefined)
...
meta_final <- mutate(meta_final,
                     AnnotationRefined_Final = case_when(
                       cluster8_1_integrated_snn_res.0.1 == 1 & AnnotationRefined_Final %in% c("Prv-CCL19","Prv-MYH11", "Prv-STEAP4" , "VSMC") ~ "Prv_VSMC secretory",
                       TRUE ~ AnnotationRefined_Final
                     ))
table(meta_final$AnnotationRefined_Final) #sanity check
...
data <- readRDS("../_Data/01_Atlas_generation/Integration_epithelial_only/epithelial_cells.rds")
data <- AddMetaData(data, meta_final)
DimPlot(data, split.by = "MenstrualCyclePhase", group.by = "AnnotationRefined_Final")
...
table(epithelial_clusters_0_1$AnnotationRefined)
...
meta_final <- mutate(meta_final,
                     AnnotationRefined_Final = case_when(
                       epithelial_clusters_0_1_res.0.05 %in% c("0","3") & AnnotationRefined_Final %in% c("glandular early-secretory","mid-secretory") ~ "glandular",
                       TRUE ~ AnnotationRefined_Final
                     ))
table(meta_final$AnnotationRefined_Final) #sanity check
...
data <- readRDS("../_Data/EndoAtlas.rds")
data <- AddMetaData(data, meta_final)
saveRDS(data,"../_Data/EndoAtlas.rds")
