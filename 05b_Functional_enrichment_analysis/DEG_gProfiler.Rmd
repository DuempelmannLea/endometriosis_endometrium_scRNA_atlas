...
RNAlistRDS005 %>% dplyr::filter(logFC > 0.5) %>% nrow()
RNAlistRDS005 %>% dplyr::filter(logFC < -0.5) %>% nrow()

RNAlistRDS005 <- RNAlistRDS005 %>% dplyr::filter(annotation_reference == "Tan")
###plotting
plot(density(RNAlistRDS005$AveExpr))
# bar plot of DEGs by condition
df <- RNAlistRDS005 %>%
  dplyr::group_by(cluster_id, stage, clustering,annotation_reference) %>%
  dplyr::summarise(DEGs = n(), median_logFC = median(logFC), median_AveExpr = median(AveExpr), median_padjloc = median(p_adj.loc)) %>%
  ungroup()
ggplot(df, aes(x=stage, y = DEGs, fill=cluster_id)) +
  geom_bar(stat='identity') +
  scale_y_continuous(expand = c(0,0)) +
  RotatedAxis() +
  xlab('Sample ID') +
  facet_wrap(~clustering,scales = "free_x", ncol = 4)
ggsave(paste0(dir_out, "Barplot_cluster_id_stage_clustering.pdf"),
  plot = last_plot(),
  width = 15, height = 6)
sum(df$DEGs)
# bar plot of DEGs by condition with AveExpr > 18.5
#There are extremely many DEGs in the clusters eF, epithelial and C7_Activated_Fibro, too many to be reliable. Median AveExpr seems to be lower in those clusters and gProfiler gave good results with a threshold of AveExpr>18.5. Therefore, I set the cut-off at AveExpr>18.5
df18_3 <- RNAlistRDS005 %>%
  dplyr::filter(AveExpr > 18.3) %>%
  dplyr::group_by(cluster_id, stage, clustering,annotation_reference) %>%
  dplyr::summarise(DEGs = n(), median_logFC = median(logFC), median_AveExpr = median(AveExpr), median_padjloc = median(p_adj.loc)) %>%
  ungroup()
df18_5 <- RNAlistRDS005 %>%
  dplyr::filter(AveExpr > 18.5) %>%
  dplyr::group_by(cluster_id, stage, clustering,annotation_reference) %>%
  dplyr::summarise(DEGs = n(), median_logFC = median(logFC), median_AveExpr = median(AveExpr), median_padjloc = median(p_adj.loc)) %>%
  ungroup()
ggplot(df18_5, aes(x=stage, y = DEGs, fill=cluster_id)) +
  geom_bar(stat='identity') +
  scale_y_continuous(expand = c(0,0)) +
  RotatedAxis() +
  xlab('Sample ID') +
  facet_wrap(~clustering,scales = "free_x", ncol = 4) +
  ggtitle("AveExpr > 18.5")
ggsave(paste0(dir_out, "Barplot_cluster_id_stage_clustering_AveExpr185.pdf"),
  plot = last_plot(),
  width = 15, height = 6)
ggplot(df18_5, aes(x=stage, y = DEGs, fill=cluster_id)) +
  geom_bar(stat='identity') +
  ...
  xlab('Sample ID')+
  ggtitle("AveExpr > 18.5") 
ggsave(paste0(dir_out, "Barplot_cluster_id_stage_AveExpr185.pdf"),
  plot = last_plot(),
  width = 6, height = 4)
sum(df18_5$DEGs)

# Count the number of genes in each combination of clustering and stage
RNAcounts <- table(RNAlistRDS005$clustering, RNAlistRDS005$stage)
pheatmap(RNAcounts)
# Create the grouped bar plot
ggplot(as.data.frame(RNAcounts), aes(x = Var1, y = Freq, fill = Var2)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(x = "Clustering", y = "Count", fill = "Stage") +
  ggtitle("Count of Genes by Clustering and Stage") +
  theme_bw() +
  theme(plot.title = element_text(size = 14, face = "bold"),
        axis.text.x = element_text(angle = 45, hjust = 1))

# Create vulcano plot
ggplot(RNAlistRDS005, aes(x = logFC, y = -log10(p_adj.loc), color = cluster_id)) +
  geom_point(data = RNAlistRDS005) +
  theme_minimal() +
  xlim(-0.8,1.3)
ggsave(paste0(dir_out, "Vulcano_logFC_padjloc_clusterid.pdf"),
  plot = last_plot(),
  width = 6, height = 4)

#scatter plot logFC, AveExpr
ggplot(RNAlistRDS005, aes(x = AveExpr, y = logFC, color = cluster_id)) +
  geom_point(data = RNAlistRDS005) +
  theme_minimal()
ggsave(paste0(dir_out, "Scatter_logFC_AveExpr_clusterid.pdf"),
  plot = last_plot(),
  width = 6, height = 4)
plot(density(log2(RNAlistRDS005$AveExpr-16.97529)))
```
